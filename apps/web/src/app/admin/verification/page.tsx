    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        {['', 'pending', 'approved', 'rejected'].map(s => (
          <button key={s} onClick={() => { setFilter(s); load(s); }} className={`px-3 py-1.5 rounded-lg text-sm font-bold ${filter === s ? 'bg-brand text-white' : 'bg-gray-200 text-gray-600'}`}>{s || 'All'}</button>
        ))}
      </div>
      <p className="text-sm text-gray-500 mb-4">{total} requests</p>
      <div className="space-y-3">
        {loading ? <p className="text-gray-400">Loading...</p> : requests.map(r => (
          <Link key={r.id} href={`/admin/verification/detail?id=${r.id}`} className="block bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition">
            <div className="flex items-center gap-3 mb-2">
              {r.avatarUrl && <img src={r.avatarUrl} alt="" className="w-10 h-10 rounded-full" />}
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400">@{r.username} &middot; {r.reqDisplayName}</p>
              </div>
              <span className={`text-xs font-bold px-2 py-1 rounded ${CATEGORY_COLORS[r.category] || ''}`}>{r.category}</span>
            </div>
