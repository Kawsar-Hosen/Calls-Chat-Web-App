...
export function emojiSrc(def: EmojiDef): string {
  if (def.src === 'bundled') return def.family === 'telegram' ? `/emoji-tg/${def.file}` : `/emoji/${def.file}`;
  return `${API_URL}/emoji/${def.family}/${def.id}.gif`;
}
...
export function FluentEmoji({ char, size = 20, family = 'fluent', className = '' }: { char: string; size?: number; family?: EmojiFamily; className?: string }) {
  const def = emojiByChar(char, family);
  if (!def) return <span aria-label={char}>{char}</span>;
  return <img className={`fluent-emoji ${className}`.trim()} src={emojiSrc(def)} alt={def.name} loading="lazy" style={{ width: size, height: size, verticalAlign: 'text-bottom' }} />;
}
