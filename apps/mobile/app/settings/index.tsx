  ...
  return (
    <SafeAreaView edges={['top']} style={[styles.safe, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <SettingRow icon="bell-outline" title={t('notification')} detail={pushState === 'ready' ? 'Enabled' : pushState === 'loading' ? 'Requesting...' : pushState === 'unavailable' ? 'Unavailable' : 'Push messages and alerts'} active={pushState === 'ready'} onPress={() => void enablePush()} rtl={isRTL} />
        ...
        <SettingRow icon="bell-badge-outline" title="Notifications" detail="Manage what you get notified about" onPress={() => router.push('/settings/notifications-settings')} rtl={isRTL} />
        <SettingRow icon="palette-outline" title="Appearance" detail="Font size, accent color" onPress={() => router.push('/settings/appearance')} rtl={isRTL} />
        <SettingRow icon="key-outline" title="Account" detail="Change password, email" onPress={() => router.push('/settings/account')} rtl={isRTL} />
        ...
        <SettingRow icon="database-outline" title="Storage & Cache" detail="Manage local data" onPress={() => router.push('/settings/storage')} rtl={isRTL} />
        <SettingRow icon="bug-outline" title="Report a Problem" detail="Send feedback or bug reports" onPress={() => router.push('/settings/report')} rtl={isRTL} />
        <SettingRow icon="information-outline" title="About" detail="App info & legal" onPress={() => router.push('/settings/about')} rtl={isRTL} />
...
const styles = StyleSheet.create({
  safe: { flex: 1 }, content: { paddingHorizontal: 16, paddingBottom: 32 },
  settingRow: { minHeight: 70, borderWidth: 1, borderRadius: 8, paddingHorizontal: 12, marginBottom: 8, alignItems: 'center', gap: 11 }, settingIcon: { width: 38, height: 38, borderRadius: 8, alignItems: 'center', justifyContent: 'center', alignSelf: 'center' }, settingTitle: { fontSize: 14, fontWeight: '800' }, settingCopy: { fontSize: 11, marginTop: 3 },
