...
function parseStoryLink(content: string | null): { authorId: string } | null {
  ...
  const match = content.match(/^\[story:([a-f0-9-]+)\]$/);
  return match?.[1] ? { authorId: match[1] } : null;
}
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <View style={[styles.header, { backgroundColor: hBg, borderBottomColor: themed ? activeTheme.border : colors.border }]}>
          ...
          <Pressable onPress={() => params.groupId ? router.push({ pathname: '/groups/[id]', params: { id: params.groupId } }) : peerId ? router.push({ pathname: '/contacts/[id]', params: { id: peerId, name, username: params.username ?? '' } }) : undefined} style={[styles.titleBtn]}>
            ...
            <View style={styles.identity}><View style={{ flexDirection: 'row', alignItems: 'center' }}><Text numberOfLines={1} style={[styles.headerName, { color: hText, fontFamily }]}>{name}</Text>{!isGroup && peerUser?.isVerified ? <VerifiedBadge category={peerUser.verifiedCategory ?? null} username={peerUser.username} displayName={name} verifiedAt={peerUser.verifiedAt ?? null} /> : null}</View><Text style={[styles.presence, { color: presenceColor }]}>{presenceLine}</Text></View>
          </Pressable>
          {!isGroup ? <>
