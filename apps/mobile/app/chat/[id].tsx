...
import { ActivityIndicator, FlatList, Image, KeyboardAvoidingView, Linking, Modal, PanResponder, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, View, useWindowDimensions } from 'react-native';
...
export default function ChatScreen() {
  const params = useLocalSearchParams<{ id: string; name?: string; username?: string; groupId?: string; groupName?: string; autoSearch?: string }>();
  const { user } = useAuth();
  ...
  const prefs = meta.prefs[params.id] ? { showTimestamps: true, showReceipts: true, showTyping: true, sound: 'default', ...meta.prefs[params.id] } : chatPrefsFor(params.id);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (latest) void api.markRead(params.id, latest.id).catch(() => undefined);
  ...
  const peerId = !isGroup && user ? messages.map((item) => item.senderId).find((senderId) => senderId !== user.id) ?? null : null;
  useEffect(() => {
    if (!peerId) return;
    let disposed = false;
    void api.presence(peerId).then((row) => { if (!disposed) { setPeerOnline(row.isOnline); setPeerLastSeen(row.lastSeenAt); } }).catch(() => undefined);
  ...
  useEffect(() => subscribe((event: SocketEvent) => {
    ...
    if (event.type === 'message.created') void api.markRead(params.id, event.message.id).catch(() => undefined);
    setMessages((current) => {
      ...
      if (event.type === 'message.created' && message.senderId === user?.id) playSound('send');
  ...
  const attachOptions: { key: string; icon: MCIconName; label: string; color: string; onPress: () => void }[] = [
    { key: 'camera', icon: 'camera-outline', label: 'Camera', color: colors.accent, onPress: () => void pickCamera() },
    { key: 'gallery', icon: 'image-multiple-outline', label: 'Gallery', color: colors.success, onPress: () => void pickGallery() },
    { key: 'document', icon: 'file-document-outline', label: 'Document', color: colors.danger, onPress: () => void pickDocument() },
  ];
  ...
  const customization = isGroup && groupInfo ? groupInfo.customization : null;
  ...
  const renderMessage = ({ item, index }: { item: Message; index: number }) => {
    ...
    const bubbleStyle = item.content && isEmojiOnly(item.content) ? styles.bubbleEmoji : item.attachments.some((a) => (a.name ?? '').startsWith('GIPHY:')) ? styles.bubbleGiphy : look ? { backgroundColor: look.background, borderColor: look.borderColor, borderWidth: look.borderWidth, borderRadius: look.radius } : { backgroundColor: mine ? colors.accent : colors.surface, borderColor: mine ? colors.accent : colors.border };
    ...
    return (
      <ReplySwipe onReply={() => { if (!item.deletedAt) { setReplyingTo(item); setEditingId(null); } }}>
  ...
  const messageList = (
    <FlatList
      ...
      ListEmptyComponent={<View style={styles.empty}><Text style={[styles.emptyTitle, { color: themed ? activeTheme.headerText : colors.text }]}>Start the conversation</Text><Text style={[styles.emptyCopy, { color: error ? (themed ? activeTheme.accent : colors.danger) : (themed ? activeTheme.time : colors.muted) }]}>{error || 'Send a message to begin.'}</Text></View>}
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={[styles.header, { backgroundColor: hBg, borderBottomColor: themed ? activeTheme.border : colors.border }]}>
          ...
          <Pressable disabled={!isGroup} onPress={() => params.groupId ? router.push({ pathname: '/groups/[id]', params: { id: params.groupId } }) : undefined} style={[styles.titleBtn]}>
            {isGroup ? <View style={[styles.groupAvatar, { backgroundColor: themed ? 'rgba(255,255,255,0.18)' : colors.accentSoft }]}><MaterialCommunityIcons name="account-group-outline" size={18} color={themed ? activeTheme.headerText : colors.accent} /></View> : <Avatar name={name} size={39} online={peerOnline === true} />}
        ...
        <Modal visible={attachOpen} transparent animationType="fade" onRequestClose={() => setAttachOpen(false)}><Pressable style={styles.sheetBackdrop} onPress={() => setAttachOpen(false)}><Pressable onPress={() => undefined} style={[styles.attachSheet, { backgroundColor: colors.surface, borderColor: colors.border }]}>{attachOptions.map((option) => <Pressable key={option.key} onPress={option.onPress} style={({ pressed }) => [styles.attachRow, { opacity: pressed ? 0.6 : 1 }]}><View style={[styles.attachIcon, { backgroundColor: `${option.color}1F` }]}><MaterialCommunityIcons name={option.icon} size={22} color={option.color} /></View><Text style={[styles.attachLabel, { color: colors.text }]}>{option.label}</Text><MaterialCommunityIcons name="chevron-right" size={20} color={colors.faint} /></Pressable>)}</Pressable></Pressable></Modal>
