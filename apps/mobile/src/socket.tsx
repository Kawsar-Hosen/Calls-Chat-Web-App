      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (type === 'message.created' || type === 'message.updated' || type === 'message.deleted' || type === 'reaction.updated') {
            ...
            emit({ type, conversationId: String(raw.conversation_id ?? ''), userId: String(raw.user_id ?? ''), ...(raw.sdp ? { sdp: String(raw.sdp) } : {}), ...(type === 'call.offer' && raw.kind ? { kind: String(raw.kind) as 'audio' | 'video' } : {}), ...(raw.caller_name ? { caller_name: String(raw.caller_name) } : {}), ...(raw.caller_username ? { caller_username: String(raw.caller_username) } : {}), ...(raw.caller_avatar ? { caller_avatar: String(raw.caller_avatar) } : {}) });
